...
class Processor():
    def __init__(self, args):
        ...
        Dataloader = DataLoader_bytrajec2_sdd
    ...
    def save_best_model(self,epoch):
        ...
        model_path= self.args.save_dir + '/' + self.args.train_model + '/' + 'best' + '_' +\
                                   str(epoch) + '.tar'
        torch.save({
            'epoch': epoch,
            'state_dict': self.net.state_dict(),
    ...
    def train_epoch(self,epoch):
        ...
        for batch in range(self.dataloader_gt.trainbatchnums):
            ...
            self.net.zero_grad()
            SRGAT_loss, full_pre_tra = self.net.forward(inputs_fw, epoch, iftest=False)
            if SRGAT_loss == 0:
                continue
            loss_epoch += SRGAT_loss.item()
            SRGAT_loss.backward()
            ...
            if batch%self.args.show_step == 0 and self.args.ifshow_detail:
                ...
                format(batch,self.dataloader_gt.trainbatchnums, epoch,SRGAT_loss.item(), end - start))
    ...
    def val_epoch(self,epoch):
        ...
        for batch in range(self.dataloader_gt.valbatchnums):
            ...
            batch_abs_gt, batch_norm_gt, shift_value_gt, seq_list_gt, nei_num = inputs_gt
            inputs_fw = batch_abs_gt, batch_norm_gt, nei_lists, nei_num, batch_split  #[H, N, 2], [H, N, 2], [B, H, N, N], [N, H]
            SRGAT_loss,full_pre_tra= self.net.forward(inputs_fw, epoch, iftest=True)
            if SRGAT_loss == 0:
                continue
